...
class TestedScrollView: NSScrollView {
    ...
    override func awakeFromNib() {
        ...
        if let docView:NSView = self.documentView as NSView! {
            ...
            willChangeValue(for: \TestedScrollView.canvasWidth)
            willChangeValue(for: \TestedScrollView.canvasHeight)
            
            let allConstraints = contentView.constraints
            contentView.removeConstraints(allConstraints)
            
            documentWidthConstraint = NSLayoutConstraint(
                item: docView, attribute: NSLayoutConstraint.Attribute.width,
                relatedBy: NSLayoutConstraint.Relation.equal,
                toItem: nil,
                attribute: NSLayoutConstraint.Attribute.width,
                multiplier: 1,
                constant: CGFloat(canvasWidth)
            )
            documentHeightConstraint = NSLayoutConstraint(
                item: docView,
